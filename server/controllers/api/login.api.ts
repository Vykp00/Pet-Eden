...
export default async function loginApi(req: Request, res: Response) {
  try {
    ...
    const checkUserEmail = await findObjectFromDB('users', query).catch(err =>
    ...
    if (
      checkUserEmail &&
      checkUserEmail.outdoc &&
      checkUserEmail.isFound == true
    ) {
      // Compare passhword with hashPassword from DB
      // The Promise return boolean value
      const checkUsrPassword = await compareHashPassword(
        usrPassword,
        checkUserEmail.outdoc.usrPassword,
      ...
      if (checkUsrPassword == true) {
        ...
        return res.status(200).json({
          message: `Welcome ${checkUserEmail.outdoc.fullName}`,
          session: req.session,
        })
      } else {
        // If Password does not match DB, return email or password incorrect with 422 - Unprocessable Entity
        return res.status(422).json({
          message: 'Email or passsword is incorrect, please try again!',
        })
      }
    } else {
      ...
      return res
        .status(422)
        .json({ message: 'Email or passsword is incorrect, please try again!' })
    }
  } catch (error: any) {
